...
import { Shield, UserPlus, Truck, User, Eye, EyeOff } from "lucide-react";
...
const ROLE_META: Record<string, {
  label: string; variant: "purple"|"info"|"default"; icon: React.ElementType;
}> = {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      if (!contentType.includes("application/json")) {
        throw new Error("Server error — check FIREBASE_ADMIN_* env vars in .env.local");
      }
    ...
    <Modal
      ...
      footer={
        <div className="flex gap-3">
          ...
          <Button className="flex-1" type="submit" form="add-staff-form" loading={saving}>
            Create account
          </Button>
        </div>
      }
    >
      <form id="add-staff-form" onSubmit={handleCreate} className="space-y-3">
        <Input
          label="Full name *"
          value={name}
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-900 focus:border-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-100 pr-12"
            />
            <button
              type="button"
...
export default function UserManagement() {
  ...
  async function toggleActive(user: AppUser) {
    ...
    await updateDoc(doc(db, "users", user.uid), {
      active: !user.active, updatedAt: serverTimestamp(),
    });
    setUpdating(null);
  }
