  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card className="text-center py-3"><p className="text-2xl font-semibold text-amber-700">{active.length}</p><p className="text-xs text-gray-500 mt-0.5">Active returns</p></Card>
        <Card className="text-center py-3"><p className="text-2xl font-semibold text-green-700">{closed.length}</p><p className="text-xs text-gray-500 mt-0.5">Closed</p></Card>
        <Card className="text-center py-3"><p className="text-2xl font-semibold text-brand-700">{withUs.length}</p><p className="text-xs text-gray-500 mt-0.5">Tyres with us</p></Card>
        <Card className="text-center py-3"><p className="text-2xl font-semibold text-blue-700">{withCEAT.length}</p><p className="text-xs text-gray-500 mt-0.5">With CEAT</p></Card>
      </div>
          ...
          <Card key={r.id}>
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 mt-0.5">{r.qty}× {r.productName} · Rs {((r as any).totalValue ?? 0).toLocaleString()}</p>
                <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
                  ...
                  {r.replacementReceivedAt && <span className="text-green-700">CEAT replacement: {formatDate(r.replacementReceivedAt)}</span>}
                </div>
              </div>
            </div>
          </Card>
